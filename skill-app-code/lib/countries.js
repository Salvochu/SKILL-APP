// Countries for the profile form's country and phone dial code selects.
// Flags are derived from the ISO 3166-1 alpha-2 code (two Unicode
// regional indicator symbols), not hand-picked emoji, so the only thing
// that can be wrong per row is the code and dial prefix themselves.
// Kosovo (XK) is a widely used but non-ISO code, its flag may not render
// on every platform; that is a Unicode gap, not a data error.
...
const RAW = [
  ...
  ["Cape Verde", "CV", "+238"], ["Chad", "TD", "+235"], ["Chile", "CL", "+56"],
  ["China", "CN", "+86"], ["Colombia", "CO", "+57"], ["Congo (DRC)", "CD", "+243"],
  ...
  ["Portugal", "PT", "+351"], ["Puerto Rico", "PR", "+1"], ["Qatar", "QA", "+974"],
  ...
  ["Sweden", "SE", "+46"], ["Switzerland", "CH", "+41"], ["Syria", "SY", "+963"],
  ["Taiwan", "TW", "+886"], ["Tajikistan", "TJ", "+992"], ["Tanzania", "TZ", "+255"],
  ["Thailand", "TH", "+66"], ["Timor-Leste", "TL", "+670"], ["Togo", "TG", "+228"],
...
export const COUNTRIES = RAW.map(([name, code, dial]) => ({
  name,
  code,
  dial,
  flag: flagEmoji(code),
...
// Longest matching dial-code prefix, so re-opening a saved phone number
// (e.g. "+971501234567") can split back into a code and a local part
// instead of dumping the whole thing into one field.
export function splitPhone(fullPhone) {
  const trimmed = String(fullPhone || "").trim();
  if (!trimmed.startsWith("+")) return { dial: "", local: trimmed };
